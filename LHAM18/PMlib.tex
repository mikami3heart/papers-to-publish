...
applying further optimization to the application to reduce the amount of elapsed time.
Therefore,
...
expected based on the system’s specifications.

Preceding studies that investigated the origin of this gap between the actual
performance and the expected performance have primarily addressed this analysis from an architecture perspective, e.g., 
...
and the other is the system workload that is actually executed on HPC systems. This is represented as machine instructions and is measured with
...
and is sometimes confusing to application users with respect to an understanding
their behavior.
...
the shorter interval statistics which can be read by a Web browser’s
...
The term "computational workload" for scientific application developers
...
In the development phase of HPC applications, practical developers
choose and deploy the numerical algorithm which requires the least
amount of arithmetic operations based on scientific considerations,
with the objective of reducing the elapsed time to obtain the desired
simulation results.
The workload in this context is named the arithmetic workload in this paper.
...
The ``Weight factor'' is used to map the original arithmetic operations to the
...
\noindent This workload formula is termed the ``application workload'' in this paper.
Equation~(\ref{eq:application-workload})
is a simple representation of the workload encountered at the application level 
using the original arithmetic,
and provides an easy-to-understand relationship between the change
in the algorithm and the resulting change in the application workload.

The application performance is evaluated based on the elapsed time as representing in the following expression.
...
Linpack performance value "flops" are reported using this workload as
\begin{align*}
...
provides a general idea of the theoretical workload requirement by a particular
application.
...
layered cache hit/miss rate, and the corresponding bandwidth to/from memory.
...
\noindent The workload measured using this approach is termed the HWPC workload,
and the performance is evaluated as HWPC.
...
In order to obtain these results, the PMlib user mode report and PMlib HWPC mode
reports were used.
% IVY case
Fig.~\ref{fig:workload-difference} shows the difference between these two parameters as a ratio.
%
% FX100 case
...
we can take exploit the related research~\cite{Hoshimoto:2015},~\cite{ccaebt:HPCAsia2018}
...
a system specific high-resolution low-a overhead timer if such timer is
...
\begin{lstlisting}[caption={basic kernels(essential lines only)}]
...
and automatic parallelization is disbled.
...
%
%
Figure~\ref{fig:fx100-gflops-user-long-R8} shows the 
...
Figure~\ref{fig:fx100-gflops-system-long-R8} shows the 
...
This performance trend for add/mult/fma is consistent with the reports
...
The performance of div/sqrt/mix shows quite different shape.
...
the L1 cache size limit is explained by the high computational destity.
\begin{table}[b]
%	\scriptsize
\caption{ratio of system/user performance for basic kernels}
\label{tab:ratio-system-user-fx100}
%	\footnotesize
\begin{tabular}{l|c|c|c|c|c|c} \hline
%	\scriptsize
type &	add	&	mult &	fma &	divi &	sqrt &	mix \\ \hline
...
and lowest at 4n+3.
Figure~\ref{fig:fx100-gflops-short-R8} highlights the fact that
...
Figure.~\ref{fig:ivy-gflops-short-R8} shows the user performance
of the same basic kernels on IVY. The stepping effect of SIMD on IVY
...
%%
\section{Conclusion}
In evaluating the application performance of HPC systems,
user perspective and system perspective are both important,
and they often exhibit different characteristics.

PMlib is an open source library that provides some key features
addressing the synthetic performance evaluation from both of
user perspective and system perspective,
as explained in
Section.~\ref{section:PMlib}
and
Section.~\ref{section:using-PMlib}.
It also provides useful information including the process level and the
thread level HWPC statistics reports,
and optional OTF file output for visualization.
PMlib is a small and portable package that facilitates
a deeper understanding of the performance model
of an application for a specific HPC system.
